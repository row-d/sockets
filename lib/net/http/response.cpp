...
void Response::send(const std::string &body, HTTPStatus status_code, const std::string &content_type)
{
    send(body, status_code, content_type, keep_alive_);
}

void Response::send(const std::string &body, HTTPStatus status_code, const std::string &content_type, bool keep_alive)
{
    if (sent_)
    {
        return;
    ...
    if (status_code == HTTPStatus::NotFound)
    {
        status_text = "Not Found";
    }
    else if (status_code == HTTPStatus::InternalServerError)
    {
