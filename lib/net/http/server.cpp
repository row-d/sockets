...
{
    response.send("Not Found", HTTPStatus::NotFound);
}

void send_server_error(Response &response)
{
    response.send("Internal Server Error", HTTPStatus::InternalServerError);
}
} // namespace
...
void HTTPServer::listen(int port, std::function<void()> on_ready)
{
